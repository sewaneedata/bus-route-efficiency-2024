...
buses <- sort( unique( bus_routes$Bus ) )

base_map <- tm_shape( franklin ) + tm_polygons( alpha = 0.5, lwd=3 )

for( i in 1:length(buses) ){
  base_map <- base_map + tm_shape( bus_routes %>% filter( Bus == buses[i] ) ) + tm_dots( col="Bus" )
}
